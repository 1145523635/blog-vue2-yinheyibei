 ...
 * @LastEditors: 银河以北
 * @LastEditTime: 2021-12-11 09:38:33
-->
<template>
  <div class="app-container">
    <div class="container">
      ...
      <div class="article-title">
        <input type="text" v-model="articleForm.article_title" @mouseover="mouseover" @mouseleave="mouseleave"
          placeholder="请输入文章标题" class="title-input" />
        <div class="split-line" ref="split-line"></div>
      </div>
      <!-- 富文编辑器 -->
      <div class="mark-down">
        <mavonEditor ref="md" v-model="articleForm.article_content" :subfield="false" :scrollStyle="true"
          @imgAdd="mdImsAdd" @change="saveMd" style="height: 100%; z-index: 1" />
      </div>
      <!-- 选项文章选项 -->
        ...
        <div class="article-form" style="margin-right: 10px">
          <el-form ref="form" :model="articleForm">
            <el-form-item label="文章分类">
              <el-select v-model="articleForm.article_classification" size="mini" style="width: 100%" clearable
                placeholder="请选择当前文章的类型">
                <el-option v-for="(item,index) in classificationOptions" :key="item.id"
                  :label="item.classification_name" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="文章专题">
              <el-select v-model="articleForm.article_special" size="mini" style="width: 100%" multiple clearable
                placeholder="请选择当前文章的专题" :multiple-limit='5'>

                <el-option v-for="(item,index) in specialOptions" :key="index" :label="item.special_name"
                  :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="文章标签">
              <el-select v-model="articleForm.article_label" size="mini" style="width: 100%" multiple clearable
                placeholder="请选择当前文章的标签" :multiple-limit='5'>
                <el-option v-for="(item,index) in labelOptions" :key="index" :label="item.label_name" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          </el-form>
        ...
        <div class="article-form">
          <el-form>
            <el-form-item label="文章封面">
              <el-upload class="upload-demo" drag :action="action" :headers="headers" :data="uploadData"
      ...
      <div class="btn-container">
        <div class="btn">
          <el-button type="info" icon="el-icon-document-checked" size="small" @click="preservationData"
            :loading='loading.preservationLoading'>保存</el-button>
          <el-button type="primary" @click="saveData" :loading='loading.saveDataLoading' icon="el-icon-circle-check"
